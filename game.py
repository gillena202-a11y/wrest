...
def weekly_menu() -> str:
    """Render a button-like action menu to guide the player."""
    ...
    options = [
        ...
        ("8", "Private coach", "+technique/mentality"),
        ("9", "Seek NIL/sponsorship", "HS seniors & college"),
        ("10", "Enter dual meet", "In season"),
        ("11", "Enter tournament", "In season"),
        ("12", "Save game", "Instant save"),
        ("13", "Quit", "Save & exit"),
    ]

    def button(num: str, label: str, desc: str) -> str:
    ...
    border = f"┌{'─' * 62}┐"
    footer = f"└{'─' * 62}┘"
    body = "\n".join(button(n, l, d) for n, l, d in options)
    hint = "Type the number or a phrase like 'train strength' or 'enter tournament'."
    helper = f"│ {hint.ljust(62)} │"
    return "\n".join(["Choose an action (buttons or free text):", border, body, helper, footer])


def interpret_command_text(choice: str) -> Optional[str]:
    """Translate free-form text into a recognized command key."""
    ...
    for key, mapped in keywords.items():
...
def process_choice(choice: str, player: Player, season: Season) -> Optional[str]:
    """Run a choice that may come from a button number or free text."""

    translated = interpret_command_text(choice) or choice
    actions = {
        "1": "technique",
        "2": "strength",
        "3": "condition",
    ...
    if translated in actions:
        return player.apply_action(actions[translated])
    if translated == "10":
...
def main() -> None:
    ...
    while True:
        ...
        print("\n" + weekly_menu())
        choice = input("Action (type number or a short phrase): ").strip()
